...
def calc_T_AP(imap, rad_arcmin, test=False, mask=None, divmap=None):
    ...
    if mask is None:
        ...
        if (np.sum(mask[inner]) == 0.) or (np.sum(mask[outer]) == 0.):
            ...
            flux_inner_std = np.sqrt((np.sum((imap[inner]*mask[inner])**2)-flux_inner**2)/np.sum(mask[inner]))
            flux_outer_std = np.sqrt((np.sum((imap[outer]*mask[outer])**2)-flux_outer**2)/np.sum(mask[outer]))
            if divmap is not None:
